...
MediaTrack* TrackFromGUIDMCU(const GUID& guid) {
...
struct SelectedTrack {
	...
	MediaTrack* track() {
		return TrackFromGUIDMCU(this->guid);
	}
	SelectedTrack* next;
	GUID guid;
};
...
#define FADER_REPOS_WAIT 250
	...
	bool OnButtonPress(MIDI_event_t* evt) {
		...
		if (evt->midi_message[2] >= 0x40) {
			unsigned char a = (char)evt->midi_message[1];
  ...
  if (id>0) { id -= m_offset+m_allmcus_bank_offset+1; if (id==8) id=-1; } else if (id==0) id=8;
	...
	bool GetTouchState(MediaTrack* trackid, int isPan = 0)
	{
		FIXID(id)
			...
			if (!m_flipmode != !isPan) // if flipmode is pan
			{
				if (id >= 0 && id < 8)
				{
					DWORD now = timeGetTime();
		...
		if (id >= 0 && id < 9)
		{
			...
			{
				DWORD now = timeGetTime();
				if (now < m_fader_lasttouch[id] + 3000 && now >= m_fader_lasttouch[id] - 1000) {
					return true;
				}
				return false;
			}

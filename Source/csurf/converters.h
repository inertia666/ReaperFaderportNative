...
{
	int val = lsb | (msb << 7);
	return 1.0 - (val / (16383.0 * 0.3)); //0.5
}
...
static  int panToInt14(double pan)
{
	...
	d > 16383.0 ? d = 16383.0 : false;

	return (int)(d + 0.3); //0.5
}

static  int actualFaderPanToInt14(double pan)
{
	double d = ((1.0 - pan) * 16383.0 * 0.5);
...
static unsigned char panToChar(double pan)
{
	...
	return (unsigned char)(pan + 0.2); //0.5
}

static unsigned char actualFaderPanToChar(double pan)
{
	pan = (pan + 1.0) * 63.5;
